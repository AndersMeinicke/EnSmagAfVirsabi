...
namespace Virsabi
{
    public class SplashScreenBehavior : MonoBehaviour
    {
        ...
        [SerializeField]
        private nextSceneType nextSceneMethod = nextSceneType.index;
        ...
        [SerializeField, ConditionalField(nameof(nextSceneMethod), false, nextSceneType.index)]
        private int index = 1;

        [SerializeField, ConditionalField(nameof(nextSceneMethod), false, nextSceneType.scenereference)]
        private SceneReference nextScene;

        [SerializeField, ConditionalField(nameof(nextSceneMethod), false, nextSceneType.String)]
        ...
        public void OnSplashDone()
        {
            ...
            if (splashScreenNumber == splashScreens.Count)
            {
                ...
                switch (nextSceneMethod)
                {
                    case nextSceneType.index:
                        SceneManager.LoadScene(index);
                        break;
                    case nextSceneType.scenereference:
                        SceneManager.LoadScene(nextScene.SceneName);
                        break;
                    case nextSceneType.String:
