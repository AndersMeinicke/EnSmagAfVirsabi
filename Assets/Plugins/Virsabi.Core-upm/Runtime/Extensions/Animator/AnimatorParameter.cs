...
#pragma warning restore 0649
		...
		public void Trigger()
		{
			...
			/*foreach (AnimatorControllerParameter param in _linkedAnimator.parameters)
			{
				if (param.type == AnimatorControllerParameterType.Trigger)
					_linkedAnimator.ResetTrigger(param.name);
			}*/

			_linkedAnimator.SetTrigger(_animParameterName);
		}

		public void Trigger(bool resetAllTriggersBeforeTriggering)
		{
			//I cannot figure out how to correctly serialize this enum. 
			if (AnimParamType != AnimatorControllerParameterType.Trigger)
			{
			...
			if (resetAllTriggersBeforeTriggering)
			{
				foreach (AnimatorControllerParameter param in _linkedAnimator.parameters)
				{
					if (param.type == AnimatorControllerParameterType.Trigger)
