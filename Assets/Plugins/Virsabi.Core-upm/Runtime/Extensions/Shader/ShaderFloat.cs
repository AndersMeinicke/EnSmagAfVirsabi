...
namespace Virsabi
{
    ...
    public class ShaderFloat
    {
        ...
        {
			_linkedMaterial.SetFloat(FloatName, value);
        }

		public float GetValue()
        {
			return _linkedMaterial.GetFloat(FloatName);
        }
	...
	public class ShaderFloatDrawer : PropertyDrawer
	{
		...
		private void UpdateFloats()
		{
			...
			for (int i = 0; i < propertyCount; i++)
			{
				...
				if (ShaderUtil.GetPropertyType(material.shader, i).Equals(ShaderUtil.ShaderPropertyType.Range)) //Also add ranges
					onlyFloats.Add(ShaderUtil.GetPropertyName(material.shader, i));
			}
